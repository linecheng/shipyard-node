var EventEmitter = require('events').EventEmitter,
  OriModem = require('docker-modem'),
  Modem = require('./shipyard'),
  ...
  Exec = require('./exec'),
  util = require('./util'),
  ext=require('./shipyard.ext'),
  extend = util.extend;

var Docker = function (opts) {
  if (!(this instanceof Docker)) return new Docker(opts);
  if (opts.serviceKey) {
    this.modem = new Modem(opts, { serviceKey: opts.serviceKey });
  } else {
    this.modem = new OriModem(opts);
  }
 ...
 */
Docker.prototype.info = function (name, callback) {
  if (typeof info === 'function')
    console.error("engine name should not be null");
 ...
 * @param  {Function} callback Callback
 */
Docker.prototype.version = function (name, callback) {
  var opts = {
    path: '/version?',
    method: 'GET',
    options: {
 ...
 * @param  {Function} callback Callback
 */
Docker.prototype.ping = function (name, callback) {
...
Docker.prototype.hello = function (opts, callback) {
  ...
  var optsf = {
    ...
    headers: {
      "X-Service-Key": "7ER9.xwdkdrVzrN.UEb1rBaRGR.1rnRJOqFe"
    }
  };
